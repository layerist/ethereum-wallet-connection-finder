...
def get_transactions(
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            log_and_print(f"Attempt {attempt}/{retries} failed: {e}")
            if attempt < retries:
                time.sleep(delay)
    return []
...
def find_connection(
    ...
    log_and_print(f"Depth {current_depth}: Found {len(transactions)} transactions", log)

    for tx in transactions:
        tx_to = tx.get("to", "").lower()
        if not tx_to or tx_to in visited:
            continue  # Skip invalid or already visited transactions
        ...
        if tx_to == address2.lower():
            ...
            return True
        if find_connection(tx_to, address2, max_depth, current_depth + 1, log, visited):
...
def main(address1: str, address2: str, max_threads: int = 4, log_file: str = "connection_log.txt"):
    ...
    with ThreadPoolExecutor(max_threads) as executor:
        future_to_thread = {
            executor.submit(find_connection, address1, address2, log=log): i for i in range(max_threads)
        }
        
        for future in as_completed(future_to_thread):
            if future.result():
                connection_found = True
                break  # Stop once a connection is found
